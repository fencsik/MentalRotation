### Plot average RT as a function of rotation, separated by samediff and sex
...
f.fig0101 <- function () {
    ...
    dt <- with(data01, tapply(RT, list(Rotation, SameDiff, Sex), mean));
    ...
    matplot(x, cbind(dt[, , 1], dt[, , 2]),
            type="n", bty="n",
            xlab="Rotation (degrees)", ylab="Reaction time (ms)");
    axis(1, x);
    axis(2);
    ...
    ## add legends
    legend("bottomright", dimnames(dt)[[3]],
           bty="n", cex=.75, pt.cex=1.2, pt.bg="white",
           col=col, pch=21, lty=1, lwd=2);
    legend("bottomright", dimnames(dt)[[2]], inset=c(0.15, 0),
           bty="n", cex=.75, pt.cex=1.2, pt.bg="white",
